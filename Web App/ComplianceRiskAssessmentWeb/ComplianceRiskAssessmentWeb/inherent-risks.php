<?php
    $title = "Inherent Risks - RegRiskTech";
...
<div class="hero">
    <div class="btn-box">
        ...
        <button id="inherentBtn" style="color:#2196F3;">Inherent Risks</button>
        </a>
...
<div id="inherentContent" class="inherentContent">
    <div class="relative"> 
        ...
        foreach($resultRowMeta as $rowMeta){
            ...
            foreach($resultRowGroup as $rowGroup){
                ...
                echo '<td title="'.$rowRisk["RiskShortDesc"].'" class="rowRisk">'.$rowRisk["RiskShortDesc"].'</td>';
                    foreach($resultBizGroup as $row){
                        ...
                        foreach($resultBiz as $colBiz){
                            ...
                            if ($queryData->rowCount() > 0) {
                                ...
                                foreach($resultData as $rowData){
                                    $gridArr[$rowNum][$colNum][12] = $rowData["CRColor"];
                                }
                                ...
                                if (isset($_SESSION["userid"])){
                                    ...
                                    $gridArr[$rowNum][$colNum][5] = "3"; //$rowData["UserID"]; // If no login then uses Default User number
                                }
                                $gridArr[$rowNum][$colNum][6] = ""; //$rowData["ColorCodeHex"];
                                $gridArr[$rowNum][$colNum][7] = ""; //$rowData["CRLabel"];
                                $gridArr[$rowNum][$colNum][8] = ""; //$rowData["IRLabel"];
                    ...
                    for($j = 0; $j < $row_cntBizMeta; $j++){
                        $colorIR = $gridArr[$rowNum][$j][6];
                        $colorCR = $gridArr[$rowNum][$j][12];
                        $colorRR = $gridArr[$rowNum][$j][13];
                        echo '<td onclick="alerting('.$gridArr[$rowNum][$j][0].','.$gridArr[$rowNum][$j][1].','.$gridArr[$rowNum][$j][2].','.$gridArr[$rowNum][$j][3].',\''.$gridArr[$rowNum][$j][9].'\',\''.$gridArr[$rowNum][$j][10].'\','.$gridArr[$rowNum][$j][5].','.$gridArr[$rowNum][$j][11].')" class="data" style="background-color:'.$colorIR.'">'.$gridArr[$rowNum][$j][8].'<span class="datatooltiptext">Inherent:<div class="box" style="background-color: '.$colorIR.';"></div><br/>Controls:<div class="box" style="background-color: '.$colorCR.';"></div><br/>Residual:<div class="box" style="background-color: '.$colorRR.';"></div><br/><a onclick="alerting('.$gridArr[$rowNum][$j][0].','.$gridArr[$rowNum][$j][1].','.$gridArr[$rowNum][$j][2].','.$gridArr[$rowNum][$j][3].',\''.$gridArr[$rowNum][$j][9].'\',\''.$gridArr[$rowNum][$j][10].'\','.$gridArr[$rowNum][$j][5].','.$gridArr[$rowNum][$j][11].')" class="updateButton" href="#">Update</a></span></td>';
                    }                 
                    echo '</tr>';
                    $rowNum += 1;   
                }
...
<script>
    ...
    //dragElement(document.getElementById("popup-content"));
    //dragElement(document.getElementById("popup-content-Headers"))
